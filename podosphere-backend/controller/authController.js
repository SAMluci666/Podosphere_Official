...
const createSendToken = (user, statusCode, res) => {
	...
	const cookieOptions = {
		expires: new Date(Date.now() + parseInt(process.env.JWT_COOKIE_EXPIRES_IN, 10) * 86400 * 1000),
		httpOnly: true,
	};
...
exports.protect = catchAsync(async (req, res, next) => {
	...
	if (await user.changesPasswordAfter(decoded.iat))
		return next(new AppError("Password has been changed, Please re-login to your account!!", 401));
...
exports.restrictTo =
	(...roles) =>
	(req, res, next) => {
		if (!roles.includes(req.user.role)) return next(new AppError("You don't have sufficient permission", 403));
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
	...
	const resetURL = `${req.protocol}://${req.get("host")}/api/v1/account/resetPassword/${resetToken}`;
	const message = `Hi ${user.name},\n\nYou requested a password reset for your Synk account. To reset your password, please click the following link:\n\n${resetURL}\n\nIf you didn't request a password reset, please ignore this email.\n\nBest regards,\nThe Synk Team`;

	try {
		await sendEmail({
			email: user.email,
			subject: "Reset Your Synk Password (valid for 10 minutes)",
			message,
		});
